...
public class TransactionResource {
    ...
    public TransactionResource(TransactionServiceImpl transactionService, JwtTokenUtil jwtTokenUtil) {
        this.transactionService = transactionService;this.jwtTokenUtil = jwtTokenUtil;
    }
    ...
    public ResponseEntity<?> getTransactionInfo(@RequestHeader("Authorization") String jwttoken, @RequestParam long vnp_Amount, @RequestParam String vnp_BankCode,
                                                @RequestParam String vnp_BankTranNo, @RequestParam String vnp_CardType, @RequestParam String vnp_OrderInfo,
                                                @RequestParam String vnp_PayDate, @RequestParam String vnp_ResponseCode, @RequestParam String vnp_TransactionNo, @RequestParam long vnp_TxnRef) {
        ...
        try {
            if (transactionService.isTransactionExisted(vnp_TransactionNo)) {
                ...
                TransactionDTO transactionDTO = transactionService.getTransactionInfo(vnp_Amount, vnp_BankCode, vnp_BankTranNo, vnp_CardType, vnp_OrderInfo,
